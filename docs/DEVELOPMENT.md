...
20. **录制分辨率必须用 `CGDisplayMode.pixelWidth/Height`,不是 `CGDisplayPixelsWide/High`**:后者在 Retina 屏返回**逻辑点**(如 1512),不是物理像素(3024)→ 录制只有 1/4 面积、清晰度严重不足。正确:`CGDisplayCopyDisplayMode(displayID).pixelWidth`。ScreenCaptureKit 的 `SCStreamConfiguration.width/height` 和截图的 `SCStreamConfiguration` 都要用物理像素。详见 DEBUG_LOG D-008。
21. **SCK 帧率是 VFR(可变帧率),不要强制节流**:配置 `minimumFrameInterval=1/60` 后,SCK 在屏幕静止时主动降帧省电(实测 ~30fps),运动时升帧。容器标称 `r_frame_rate=120` 是 ProMotion 屏的误导值,实际平均 30-60fps 是正常的。PTS 节流会把帧率砍更低,已验证有害无益,不要加。屏幕录制 30-60fps 完全够用。
